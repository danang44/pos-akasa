class Dashboard {
    ...
    load(filter) {
        ...
        this.iconPathString2 = `<svg width="152" height="427" viewBox="0 0 152 427" xmlns="http://www.w3.org/2000/svg">
        ...
        this.searchBox = `<div class="input-group mb-3">
        <span class="input-group-text dropdown" id="menu-main"><span style="cursor:pointer" id="btn-sidenav">&#9776;</span></span>
        <input type="text" id="hubdatsearch" name="hubdatsearch" class="form-control" placeholder="Search Places">
        <span class="input-group-text"><i class="bx bx-search-alt-2" style="font-size:24px"></i></span>
        ...
        */
        this.content1_ = `<div class="row">
            <div class="col-12 col-lg-3 search-wrap">`+ this.searchBox + `</div>
            
            
            <div class="col-12 col-lg-3">
                <div class="row" style="overflow:auto">
                <div class="col"><select class="form-control" style="width: 100%;height:48px;" id="find_group_nm" multiple="multiple" placeholder="d"></select></div>
                </div>
            </div>
            <div class="col-12 col-lg-3 find_bus">
                <div class="row" style="overflow:auto">
                <div class="col"><select class="form-control" style="width: 100%;height:48px" id="find_bus"></select></div>
                </div>
            </div>
            
            <div class="col-12 col-lg-3" style="display:none">
                <div class="row" style="overflow:auto">
                <div class="col"><select class="form-control" style="width: 100%;height:48px" id="edit_trayek"></select></div>
                </div>
            </div>
        ...
        // $(document).on('click','ul.metismenu li a',function(e){
        //   e.preventDefault;
        //   var $this = $('ul.metismenu li a');
    ...
    setBusTooltip(busMarker, item) {
        ...
        var infoVehicle = '<table>' +
            ...
            //'<tr><td>Rute</td><td>'+((item.route_id!=null)?thisClass.routes.filter(function c(rute){ return rute.id == item.route_id })[0].name : '-')+'</td></tr>'+
    ...
    loadDataPetugas() {
        var thisClass = this;
        if (typeof thisClass.baseUrl == 'undefined') thisClass.baseUrl = 'https://mitradarat-fms.dephub.go.id';
        $.ajax({
            type: "POST",
            url: thisClass.baseUrl + '/main/ajax/loadDataPetugas',
            data: {
                [thisClass.csrfName]: thisClass.csrfHash,
                posko_id: Call_.currentPosko,
                satpel_id: Call_.currentSatpel,
                petugas_id: Call_.currentPetugasid
            },
            //dataType: "json",
            success: function (response) {

                thisClass.DataPetugas = $.parseJSON(response);
                thisClass.fillPetugas(thisClass.DataPetugas);
    ...
    fillPetugas(ret) {
        ...
        var thisClass = this;
        console.log(thisClass.DataPetugas);
        var element = '';
        var tot = 0;
        ret.filter(function (el) { return el.name.toLowerCase().includes($('#cariuser').val().toLowerCase()) || el.lokasi.toLowerCase().includes($('#cariuser').val().toLowerCase()) }).forEach(function (item, index) {
              ...
              <a href="#">
                  <div class="d-flex align-items-start">
                      ...
                      <div class="flex-grow-1 overflow-hidden">
                          ...
                          <p class="text-truncate mb-0">${(item.lokasi == null) ? `-` : item.lokasi}</p>
                      </div>
        ...
        $('.tot-user').html(ret.filter(function (el) { return el.name.toLowerCase().includes($('#cariuser').val().toLowerCase()) || el.lokasi.toLowerCase().includes($('#cariuser').val().toLowerCase()) }).length);
        $('#list-petugas').html(element);
    }
    ...
    loadDataPetugasold() {
        ...
        $.ajax({
            type: "POST",
            url: thisClass.baseUrl + '/main/datapetugas',
            data: {
                [thisClass.csrfName]: thisClass.csrfHash
            },
            //dataType: "json",
            success: function (response) {
                $('#list-petugas').html(response);
            }
        });
    }

    activateCall(completion = null) {
        var thisClass = this;
        thisClass.pageLoading('activate Video Conference Room...');
        thisClass.api = new JitsiMeetExternalAPI(thisClass.domain, thisClass.options);
        $('#call-modal').modal('show');
        thisClass.loadDataPetugas();
        thisClass.api.addEventListener('videoConferenceJoined', function (e) {
            thisClass.api.executeCommand('avatarUrl', thisClass.options.userInfo.avatarURL);
            thisClass.displayName = e.displayName;
            thisClass.roomName = e.roomName;
            Swal.close();
            //thisClass.makeGroupCall();
            thisClass.showListPetugas();
            if (completion != null) { completion(); }
        });

        thisClass.api.addEventListener('readyToClose', function (e) {
            thisClass.api.dispose();
            $('#call-modal').modal('hide');
        });

        thisClass.participants = [];
        thisClass.api.addEventListener('participantJoined', function (e) {
            console.log('participants--');
            $('.call-this-user').removeClass('rise-shake');
            //thisClass.participants.push(e);
            console.log(thisClass.participants);
        })
    ...
    deactivateCall() {
        ...
        try {
            ...
            thisClass.api.executeCommand('endConference');
            //thisClass.api.executeCommand('hangup');
            // thisClass.api.executeCommand('removeBreakoutRoom');
            //
        } catch (e) {
            //console.log(e);
        }
    ...
    makeGroupCall() {
        ...
        $.ajax({
            type: "POST",
            url: thisClass.urlGroupCall,
            data: {
                [thisClass.csrfName]: thisClass.csrfHash,
                sender: thisClass.displayName, room: thisClass.roomName, handler: ''
            },
    ...
    makeCall(petugas_id, item) {
        var thisClass = this;
        item.addClass('rise-shake');
        $.ajax({
            type: "POST",
            url: thisClass.urlCall,
            data: {
                [thisClass.csrfName]: thisClass.csrfHash,
                sender: thisClass.displayName, room: thisClass.roomName, handler: '',
