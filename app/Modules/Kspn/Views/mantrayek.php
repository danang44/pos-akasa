            ...
            <div class="card-body" style="position: relative;min-height: 800px !important;">
                ...
                <div id="widget-add-trayek" class="widget-box">
                    ...
                    <div class="form-in">
                        ...
                        <input id="input-route-name" type="text" class="form-control" placeholder="Nama trayek, ex: Langgur - Tetoat" <?= ($rules->i != "1" ? "disabled" : "") ?>>
                    </div>
...
<script type="text/javascript">
    $(function() {
        ...
        Dashboard.vCurLatLng = [-1.0133648, 113.8965494];
        ...
        $(document).on('click', 'body:not(.dropdown-items)', function() {
            ...
            .on('click', '.rename-point', function(e) {
                e.preventDefault();
                const edit = $('.rename-point');
                const index = edit.index($(this));

                if (T.points[index] !== undefined) {
                    var item_point_val = $(this).closest('.item-point').find('input.input-point').val();
                    let item_point_prompt = prompt("Rename Point ini ", item_point_val);
                    if (item_point_prompt != null) {
                        $(this).closest('.item-point').find('input.input-point').removeClass('edited');
                        $(this).closest('.item-point').find('input.input-point').val(item_point_prompt);
                        $(this).closest('.item-point').find('span').html(item_point_prompt);
                        console.log(T.points[index].data.bs_nm);
                        T.points[index].data.bs_nm = item_point_prompt;
                        console.log(T.points[index].data.bs_nm);
                    }
                }

                $(this).closest('.item-point').find('input').hide();
                $(this).closest('.item-point').find('span').show();
            }).
        ...
        }, 500)).on('click', '.dropdown-items', function(e) {
            e.preventDefault();
    ...
    const saveForm = (data) => {
        ...
        $.ajax({
            type: "POST",
            url: Dashboard.baseUrl + '/kspn/action/mantrayek_save',
            data: data,
            dataType: "json",
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#save-trayek', function(e) {
            ...
            if ($('#input-route-code').val() == "" || $('#input-route-code').val() == null) {
                Swal.fire('Peringatan', "Kode trayek harus terisi", 'warning');
            } else if ($('#input-route-name').val() == "") {
                Swal.fire('Peringatan', "Nama trayek harus terisi", 'warning');
        ...
        $("#input-route-code").select2({
            ...
            ajax: {
                url: Dashboard.baseUrl + '/kspn/ajax/kor_select_get',
                dataType: 'json',
                quietMillis: 500,
                delay: 500,
                data: function(param) {
        ...
        let columns = [{
            ...
            },
            {
                ...
                render: function(a, type, data, index) {
                    ...
                    if (role_code == 'daj') {
                        if (auth_otorisasi == '1') {
                            button += '<button class="btn btn-sm btn-outline-success otorisasi" data-id="' + data.id + '" title="Otorisasi">\
