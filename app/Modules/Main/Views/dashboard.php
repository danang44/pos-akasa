...
<style>
    ...
    .call-this-user-dashboard,
    .map-this-user-dashboard {
        cursor: pointer;
    }
...
<script>
    ...
    $(document).ready(function() {
        try {
            ...
            const socket = io.connect("https://stream.nginovasi.id:5002", {
                secure: true,
                transports: ["websocket"]
            });
            const socket_ptis = io.connect("https://gps.brtnusantara.com:5758", {
                secure: true,
                transports: ["websocket"]
            });
            ...
            Dashboard.g_street = L.tileLayer('https://{s}.google.com/vt/lyrs=m&x={x}&y={y}&z={z}', {
                maxZoom: 18,
                subdomains: ['mt0', 'mt1', 'mt2', 'mt3']
            });

            Dashboard.g_hybrid = L.tileLayer('http://{s}.google.com/vt/lyrs=s,h&x={x}&y={y}&z={z}', {
                maxZoom: 18,
                subdomains: ['mt0', 'mt1', 'mt2', 'mt3']
            });

            Dashboard.g_terrain = L.tileLayer('http://{s}.google.com/vt/lyrs=p&x={x}&y={y}&z={z}', {
                maxZoom: 18,
                subdomains: ['mt0', 'mt1', 'mt2', 'mt3']
            });

            Dashboard.g_sat = L.tileLayer('http://{s}.google.com/vt/lyrs=s&x={x}&y={y}&z={z}', {
                maxZoom: 18,
                subdomains: ['mt0', 'mt1', 'mt2', 'mt3']
            });

            var baseLayers = {
                "Street": Dashboard.g_street,
                "Hybrid": Dashboard.g_hybrid,
                "Terrain": Dashboard.g_terrain,
                "Satellite": Dashboard.g_sat
            };
            ...
            function loadRoute(item) {
                ...
                $.each($.parseJSON(item.points), function(index2, point) {
                    ...
                    if(item.color==null) item.color='#000000';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response.data, function(index, item) {
                            item.routename = Dashboard.routes.filter(function c(rute) {
                                return rute.id == item.route_id
                            });
                            ...
                            if (ret) {
                                if (ret.lat != null) {
                                    if (typeof vehicleMarkers[ret.gps_sn] != 'undefined') {
                                        ...
                                        ret.routename = Dashboard.routes.filter(function c(rute) {
                                            return rute.id == ret.route_id
                                        });
            ...
            function createAndStorePolygon(angle, coorPolygon, addedPolygon, newShelter, oldShelter, cases, grouping) {
                if (isInsideAngle(currentAngle, angle) && cases != 'case-2') {
                    ...
                    if (currentPolygon.length >= 2) {
                        if (first == last) {
                            poly = L.polygon(currentPolygon, {
                                color: 'red',
                                data: {
                                    "newshelter": newShelter,
                                    "oldshelter": oldShelter
                                }
                            }).addTo(layerPolyGroup);
                            ...
                            tempPoly[x] = {
                                ...
                                'polygon': currentPolygon.map(function(x) {
                                    return x.lat + ' ' + x.lng;
                                }).join(', '),
                                os: oldShelter.lat,
                                ns: newShelter.lat,
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (ret.success == true) {
                            ...
                            vehicleMarkers[dataparam.gps_sn].options.data.routename = Dashboard.routes.filter(function c(rute) {
                                return rute.id == dataparam.route_id
                            });
                    ...
                    success: function(response) {
                        ...
                        for (index in ret.routes[0].legs[0].steps) {
                            ...
                            waypoints[index] = L.marker(vLatLng, {
                                ...
                                '<br/> AVG Speed ' + (distance_ / duration_).toFixed(2) + ' km/hr', {
                                    permanent: false
                                });
                    ...
                    success: function(response) {
                        if (response.data.length > 0) {
                            ...
                            Dashboard.markerAduan[item.id] = L.marker(L.latLng(item.lat, item.lon), {
                                ...
                                data: item,
                                draggable: false,
                            }).bindPopup(aduanContent, {
                                maxWidth: 560
                            }).addTo(layerGroupAduan);
            ...
            Dashboard.pulseIcon = L.icon.pulse({
                iconSize: [10, 10],
                color: vColor,
                fillColor: vColor
            });
            ...
            $(document).ajaxStop(function() {
                    ...
                    for (var key in sourceSatpelData) {
                        if (sourceSatpelData.hasOwnProperty(key)) {
                            Object.assign(overlays, {
                                [`${sourceSatpelData[key]["nama"]} (${sourceSatpelData[key]["layer"].getLayers().length})`]: sourceSatpelData[key]["layer"]
                            });
                        }
                    }
                ...
                function ssthis(element) {
                    var rect = element.getBoundingClientRect();
                    console.log("screenshot map element");
                    console.log(rect.top, rect.right, rect.bottom, rect.left);
                }

                ssthis(document.querySelector('#map'));
            });
