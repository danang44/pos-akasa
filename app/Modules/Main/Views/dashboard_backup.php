...
<style>
  ...
  @media screen and (max-width: 600px) {
      div.leaflet-top div.leaflet-left{
        top:70px;
      }
  }*/
</style>
...
<div id="map" style="min-height:100%;height:100%;width:100%;position: absolute;top:0px;"></div>
<?php
/*
...
<script>
  $(document).ready(function () {
            ...
            $.ajax({
              ...
              success:function(response){
                ...
                $.each(ret.data,function(index,item){
                    ...
                    createVehicleMarker(item,colorGroup[item.group_nm]);
                });
                    ...
                    if(ret){
                      if(ret.lat!=null){
                        if(typeof vehicleMarkers[ret.gps_sn]!='undefined'){
                          // pindah lokasi bus
                          try{
                            //console.log('move to last update location '+ret.gps_sn+' '+ret.stime);
                          ...
                          createVehicleMarker(ret,colorGroup[ret.group_nm]);
                        }
                      }
                    }
               ...
               //console.log(vdataz.results);
                ...
                $('#find_bus').on('select2:select', function (e) {
                      ...
                      var vLatLng = [parseFloat(data.lat),parseFloat(data.lon)];
                      map.flyTo(vLatLng,10);
                      //vehicleMarkers[data.gps_sn].openPopup();
                });
                ...
                if(ret.ResponseCode==1){
                    ret.Data.forEach(function(item,index){
                        ...
                        if(vehicleMarkers.hasOwnProperty(item.gps_sn)){
                            ...
                            createVehicleMarker(item,randomColor);
    ...
    function createVehicleMarker(item,randomColor){
        // var busIconURL = encodeURI("data:image/svg+xml," + iconPathString.replace('#000000','#'+randomColor)).replace('#','%23');
        var busIconURL = encodeURI("data:image/svg+xml," + iconPathString2.replaceAll('#FF1744','#'+randomColor)).replaceAll('#','%23');
        var busIconPath = L.icon({
          iconUrl: busIconURL,
    ...
    function setBusTooltip(busMarker,item){
      ...
      var infoVehicle = '<table>'+
        ...
        '<tr><td>No. Aset</td><td>'+item.no_aset+'</td></tr>'+
        '<tr><td>Tipe</td><td>'+item.car_type+'</td></tr>'+
        '<tr><td>acc</td><td>'+item.acc+'</td></tr>'+
        '<tr><td>Angle</td><td>'+item.direction+'</td></tr>'+
        '<tr><td>Nama Driver</td><td>'+item.driver_nm+'</td></tr>'+
        ...
        '<tr><td>Last Updated</td><td>'+$.timeago(item.stime)+' <br/>'+item.stime+'</td></tr>'+
        '</table>';
    ...
    function createAutoZone(e){
      ...
      $.each(ret._latlngs,function(index,item){
        ...
        L.circle(item, {color: 'green', weight: 16, fillOpacity: 0.9}).bindPopup(distance+' ['+index+']').addTo(map);
          }else if(index===ret._latlngs.length-1){
        console.log(index);
        console.log(item);
