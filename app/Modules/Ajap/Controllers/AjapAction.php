...
namespace App\Modules\Ajap\Controllers;

use App\Modules\Ajap\Models\AjapModel;
use App\Core\BaseController;

class AjapAction extends BaseController
{
    private $ajapModel;
    private $armadaTipe;
    ...
    function mantripajap_save()
    {
        parent::_authInsert(function () {
            ...
            $data["routes"] = implode(",", array_map(function ($arr) {
                if ($arr["bs_id"] == null) {
                    unset($arr['enc']);
                    $this->ajapModel->base_insert($arr, "bus_stop");
                    return str_pad($this->db->insertID(), 5, "0", STR_PAD_LEFT);
                } else {
                    $bs_id = $arr['bs_id'];
                    unset($arr['bs_id']);
                    unset($arr['enc']);
                    $this->ajapModel->base_update($arr, "bus_stop", ['bs_id' => $bs_id]);
                    return str_pad($bs_id, 5, "0", STR_PAD_LEFT);
                }
            }, $data['routes']));

            if ($data['id'] == "") {
                $this->ajapModel->base_insert($data, "fleet_routes");
            } else {
                $this->ajapModel->base_update($data, "fleet_routes", ["id" => $data['id']]);
            }
